...
avian <- read.csv("D:\\Program\\stepic\\org.stepik.math.statistics\\Rcourse-master\\Rcourse-master\\R programming\\avianHabitat.csv")

avian %>% 
  mutate(Site = factor(str_replace(Site, "[:digit:]+", ""))) %>% 
  group_by(Site, Observer) %>% 
  select(Site, Observer, contains("Ht")) %>% 
  summarise_each(.,funs(asd)) %>% 
  filter(MaxHt >= 5) 
asd = function(x){sum(x>0)}
