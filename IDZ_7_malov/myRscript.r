...
asm<-sum((x-mean)^3)/length(x)/var^(3/2); print (asm) #ассиметрия
exc<-sum((x-mean)^4)/length(x)/var^2-3; print (exc)  #эксцесс
a<-3.21; b<-5.89
...
# или: nu<-c(18,17,15) #частоты элементов
p1<-array(dim = r)
